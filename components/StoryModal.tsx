...
const StoryModal: React.FC<StoryModalProps> = ({ story, isOpen, onClose, onNewStory, isStreaming = false }) => {
  ...
  const handleToggleAudio = async () => {
    ...
    try {
      ...
      alert("Не удалось озвучить сказку. Дождитесь завершения генерации или проверьте ключ.");
    } finally {
        ...
        <GlassCard className="flex flex-col h-full bg-[#1a1b4b]/90 border-indigo-500/30 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto min-h-0 pr-2 custom-scrollbar space-y-4">
            <p className="text-lg leading-relaxed text-indigo-50 whitespace-pre-line font-light">
              {story.content}
              {isStreaming && (
                <span className="inline-block w-2 h-5 ml-1 bg-purple-400 animate-pulse align-middle" />
              )}
            </p>
          ...
          <div className="flex-none mt-6 pt-4 border-t border-white/10 flex flex-col sm:flex-row gap-3 justify-between items-center">
             <div className="flex items-center gap-2 w-full sm:w-auto bg-white/5 rounded-xl p-1 pr-3">
               ...
               <span className="text-xs font-medium text-indigo-200 hidden sm:inline">
                 {isStreaming ? 'Магия в процессе...' : (isLoadingAudio ? 'Подготовка...' : (isPlaying ? 'Играет сказка' : 'Озвучить'))}
               </span>
             </div>

            <div className="flex gap-3 w-full sm:w-auto">
              <Button variant="secondary" onClick={onClose} disabled={isStreaming} className="flex-1 sm:flex-none justify-center">
                Закрыть
              </Button>
              <Button onClick={onNewStory} disabled={isStreaming} className="flex-1 sm:flex-none justify-center">
                ✨ Новая
              </Button>
            </div>
          </div>
